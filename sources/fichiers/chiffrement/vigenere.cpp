  ...
  while (42) { // l'utilisateur souhaite continuer à chiffrer en Vigenère
    ...
    while (42) {
      ...
      if (choise == 1) {
        ...
        cout << chiffre_vigenere(input, lib, lib_size, key);
        cout << "\033[0;0m" << endl;
      }
      else {
        cout << endl << "Votre texte déchiffré : :\033[1;35m" << endl;
        cout << dechiffre_vigenere(input, lib, lib_size, key);
        cout << "\033[0;0m" << endl;
      }
      cout << endl << endl;
...
int key_check(string & key, const char * lib) { // cette fonction vient vérifier que la clef est correcte ( plus de 3 caractères différents )
  int i = 0;
  int length = key.size() - 1; // le but est de ne pas aussi prendre le '\n' qui est un caractère
...
string chiffre_vigenere(string & text, char const * lib, int lib_size, string & key) {  
  stringstream out;
  ...
  while (text[i_text] != '\0') { // on s'assure ici que tout le texte est chiffré
    if ((tmp = my_isin(text[i_text], lib)) >= 0) { // chiffrement si le caractère est dans l'alphabet choisi
      tmp += my_isin(key[i_key], lib);
      tmp %= lib_size;
      out << lib[tmp];
    } 
...
string dechiffre_vigenere(string & text, char const * lib, int lib_size, string & key) {
  stringstream out;
  int i_key = 0;
  int i_text = 0;
  int tmp = 0;
  ...
  while (text[i_text] != '\0') {
    tmp = my_isin(text[i_text], lib);
    tmpK = my_isin(key[i_key], lib);
    if ((tmpK > tmp) && tmp >= 0) {
