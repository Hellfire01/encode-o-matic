...
int V_102_user_choices () {
  ...
  while (42) {
    std::cout << "Vous avez choisit d'utiliser la méthode de chiffrement \"102V\"" << std::endl;
    c_str("Note :", 'y');
    std::cout << " il sagit d'une combinaison du 102 et du Vigenère" << std::endl;
...
void V_102_choices (int choice) {
  ...
  crypt.add_to_list(v);
  while (42) {
    std::cout << "Vous avez choisit d'utiliser le 102V" << std::endl;
    if (choice == 1) {
      std::cout << "Ici, c'est l'alphabet le plus complet qui est utilisé ( le n°4 ) par défaut" << std::endl;
    }
    if (choice == 1) {
      std::cout << "Le 102V chiffrement utilise un total de 3 clefs, une matrice, une base et une passphrase" << std::endl;
      c_str((char*)"Note : ", 'y');
      std::cout << "Il est évident qu'avoir 4 nombres différents pour la matrice garantit une meilleure sécurité" << std::endl;
      c_str((char*)"Note : ", 'y');
